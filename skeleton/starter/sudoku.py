...
def backtrack(assignment, csp):
    if checkComplete(assignment):
        ...
        for value in csp.domain[key]:
            csp.board[key] = value
            updatedConnections = csp.forwardChecking(key)

            if updatedConnections != False:
                if backtrack(assignment | {key : value}, csp) != False:
                    return csp.board
                else:
                    # Restore Domains
                    for connKey in updatedConnections:
                        csp.domainLen[connKey] += 1
                        csp.domain[connKey].add(value)
        csp.board[key] = 0
        return False
